...
class WeightGenerator(nn.Module):
    def __init__(self, feature_channel: int = 768):
        super().__init__()

        self.feature2weight: nn.Module = nn.Sequential(
            nn.AdaptiveAvgPool2d((5, 5)),
            nn.ConvTranspose2d(feature_channel, 768, kernel_size=5, groups=2, stride=2),  # 5*2 + 3 = 13
            nn.LeakyReLU(inplace=True),
            nn.ConvTranspose2d(768, 384, kernel_size=5, stride=2),  # 13*2 + 3 = 29
            nn.LeakyReLU(inplace=True),
            nn.ConvTranspose2d(384, 384, kernel_size=5, stride=2),  # 29*2 + 3 = 61
            nn.LeakyReLU(inplace=True),
            nn.ConvTranspose2d(384, 128, kernel_size=5, stride=2),  # 61*2 + 3 = 125
            nn.LeakyReLU(inplace=True),
            nn.ConvTranspose2d(128, 32, kernel_size=5, stride=2),  # 125*2 + 3 = 253
            nn.LeakyReLU(inplace=True),
            nn.ConvTranspose2d(32, 16, kernel_size=5),  # 253 + 3 = 256
        )

    def forward(self, feature):
        weight = self.feature2weight(feature)
        return weight
